...
//학생성적정보 담을 수 있는 자료형 틀 만들기
//맴버변수 + 맴버 매서드 ( 맴머변수를 사용하는 매서드)
//맴버매서드는 객체가 생성된 이후부터 사용가는하다.

//type 정의함 주의사항: 변수가 만들어진 것이 아니다.
public class Score {

	// 맴버변수 (인스턴스 변수) / new 할때만다 만들어지는 변수
	private String name;
	private int kor;
	private int eng;
	private int math;
	...
	// 오버로딩이란 매서드의 이름은 동일하고 매개변수가 다른 경우를 말한다. ( 개수, type이 다른 것을 의미)
